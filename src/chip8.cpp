...
{
  char buffer;
  std::ifstream gameCode (gameName);
  if(gameCode.is_open()){
    uint16_t i = 0x200;
...
{
  seed++;
  std::mt19937 mt(seed); //gerador mersene twist, inicializado com seed = 42
  ...
  {    
    ...
    case 0xC000: // RND Vx, byte
      randNumber = linear( mt ); //inteiro pseudo-aleatório
      V[dst] = randNumber & kk;
    break;
    case 0xD000: 
      ...
      switch (opcode & 0x00FF)
      {
        case 0x0007: // LD Vx, DT
          if(delayTimer == 0)
            ...
            V[dst] = (uint32_t)((delayTimer - SDL_GetTicks())/20); 
        break;    
        case 0x000A: // LD Vx, K
          bool keyPressed;
          do
        ...
        case 0x0015: // LD DT, Vx
          delayTimer = (uint32_t)(V[dst]*20) + SDL_GetTicks();
...
void Chip8::updateTimers()
{
  ...
  if(SDL_TICKS_PASSED(SDL_GetTicks(), delayTimer))
    delayTimer = 0;
